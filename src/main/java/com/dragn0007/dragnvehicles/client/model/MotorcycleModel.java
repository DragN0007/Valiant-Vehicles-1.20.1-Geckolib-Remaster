...
public class MotorcycleModel<T extends AbstractVehicle> extends DefaultedEntityGeoModel<T> {

    public MotorcycleModel() {
        super(new ResourceLocation(ValiantVehiclesMain.MODID, "motorcycle"), false);
    }

    @Override
    public ResourceLocation getTextureResource(T animatable) {
        return ValiantVehiclesMain.id("textures/entity/motorcycle/" + animatable.getColor().toString().toLowerCase() + ".png");
    }
    ...
    public void setCustomAnimations(T animatable, long instanceId, AnimationState<T> state) {
        ...
        processor.getBone("front_wheel").setRotX(-rot);
        processor.getBone("back_wheel").setRotX(-rot);
        processor.getBone("front_wheel").setRotY(turn);
        processor.getBone("handles").setRotY(turn);
    }
